...
import { releases, genres, releaseTypes } from '@/lib/data'
...
function AlbumGridCard({ release }: { release: typeof releases[0] }) {
      ...
      <div className="p-4">
        ...
        <div className="flex gap-1.5">
          {[
            ...
            <a key={i} href={p.href}
              className="flex-1 text-center py-2 glass rounded-lg text-xs text-white/40 hover:text-white transition-colors border border-transparent hover:border-white/10">
              {p.label}
            </a>
          ))}
        </div>
...
function AlbumListRow({ release, index }: { release: typeof releases[0]; index: number }) {
  const accent = genreColors[release.genre] || '#d4af37'
  const letters = release.title.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {[release.streaming.spotify, release.streaming.apple].map((href, i) => (
          <a key={i} href={href} className="text-white/25 hover:text-white transition-colors">
      ...
      <div className="relative max-w-7xl mx-auto px-5 md:px-8 pt-32 pb-24">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="text-5xl md:text-6xl font-black mb-3">
                Music <span className="text-gold-gradient">Catalog</span>
              </h1>
        ...
        >
          {streamingPlatforms.map(p => (
            <a
              key={p.name}
              href="#"
              className="glass px-4 py-2 rounded-full text-xs font-bold tracking-wider flex items-center gap-2 hover:bg-white/8 transition-colors border border-white/5 hover:border-white/12"
              style={{ color: p.color }}
            >
              {p.icon} {p.name}
            </a>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`btn-glass px-4 py-3.5 rounded-full text-sm font-semibold flex items-center gap-2 ${showFilters ? 'border-gold/30 text-gold' : ''}`}
            >
            ...
            <div className="glass rounded-full p-1 flex gap-1">
              <button
                ...
                className={`p-2.5 rounded-full transition-all ${view === 'grid' ? 'bg-gold text-black' : 'text-white/40 hover:text-white'}`}
              >
                <LayoutGrid size={15} />
              </button>
              <button
                onClick={() => setView('list')}
                className={`p-2.5 rounded-full transition-all ${view === 'list' ? 'bg-gold text-black' : 'text-white/40 hover:text-white'}`}
              >
                <List size={15} />
              </button>
            </div>
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-white/30 text-sm">
            Showing <span className="text-white font-bold">{filtered.length}</span> releases
          </span>
        </div>
